...
def run(request: ArchitectureRequest, ui, workspaces_root: Path | None = None) -> dict:
    ...
    with ui.progress("Encoding graph and running analysis", total=3) as advance:
    ...
    output_path = Path(request.output_markdown_path).expanduser().resolve()
    report_path = write_reports(workspace, output_path, report_markdown)
    ...
    _write_provenance(workspace, report_path, output_path)
